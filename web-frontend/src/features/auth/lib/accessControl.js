...
export function isMainWebRole(role) {
  return WEB_NAV_ROLES.includes(normalizeRole(role))
}
...
export function canCreateCase(user) {
  return WEB_NAV_ROLES.includes(normalizeRole(user?.role))
}
...
function getRoleLandingRouteName(role) {
  return ROLE_LANDING_ROUTE_MAP[normalizeRole(role)] || 'access-restricted'
}
...
function getDashboardNavItemsForUser(user) {
  ...
  const items = DASHBOARD_NAV_ITEMS.filter((item) => item.allowRoles.map(normalizeRole).includes(userRole))
...
export function isRouteAllowedForUser(user, to) {
  ...
  const userRole = normalizeRole(user.role)
  return matchedRoleGuards.every((allowRoles) => allowRoles.map(normalizeRole).includes(userRole))
}
