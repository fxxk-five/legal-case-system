...
const ERROR_CODE_MESSAGES = {
  ...
  FORBIDDEN: '无权执行该操作。',
  TENANT_ACCESS_DENIED: '无权访问当前机构资源。',
  ...
  UNSUPPORTED_MEDIA_TYPE: '文件格式不支持，请更换后重试。',
  ...
  SMS_CODE_RATE_LIMITED: '验证码请求过于频繁，请稍后重试。',
...
function mapLocToLabel(loc = []) {
  ...
  const fieldMap = {
    ...
    code: '验证码',
    token: '邀请码',
    phone_verification_token: '验证码校验令牌',
...
function mapValidationIssue(issue) {
  ...
  if (issue.type === 'string_too_short') {
    return issue.ctx?.min_length
      ? `${label}至少需要 ${issue.ctx.min_length} 个字符。`
      : `${label}长度过短。`
  }
  if (issue.type === 'string_too_long') {
    return issue.ctx?.max_length
      ? `${label}不能超过 ${issue.ctx.max_length} 个字符。`
      : `${label}长度过长。`
  }
...
export function extractFriendlyError(error, fallback = '操作失败') {
  ...
  return resolveFriendlyErrorFromPayload(error.response.data, fallback, error.response.status)
}
