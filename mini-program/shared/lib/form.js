...
export const REQUEST_FAILED_MESSAGE = "请求失败。"
export const NETWORK_ERROR_MESSAGE = "网络连接失败，请稍后重试。"
export const SERVER_ERROR_MESSAGE = "服务器暂时出现问题，请稍后重试。"
...
const ERROR_CODE_MESSAGES = {
  ...
  AI_TASK_NOT_FOUND: "AI 任务不存在或已结束。",
  AI_ANALYSIS_NOT_FOUND: "AI 分析结果不存在。",
  AI_OPERATION_NOT_ALLOWED: "当前角色无权执行该 AI 操作。",
...
export function validateTenantCode(value, required = false) {
  ...
  if (!/^[a-z0-9_-]+$/.test(text)) return "租户编码只能使用小写字母、数字、下划线和中划线"
  return ""
}
...
function mapValidationIssue(issue = {}) {
  ...
  if (issue.type === "string_too_short") {
    return `${label}至少需要 ${issue.ctx?.min_length || 1} 个字符`
  }
  if (issue.type === "string_too_long") {
    return `${label}不能超过 ${issue.ctx?.max_length || 0} 个字符`
  }
...
function resolveNativeMiniProgramError(errMsg, fallback) {
  const text = String(errMsg || "").trim()
  if (!text) {
    return fallback
  }
  if (text.includes("request:fail")) {
    return "网络连接失败，请确认后端服务已启动后重试"
  }
  if (text.includes("uploadFile:fail")) {
    return "文件上传失败，请稍后重试"
  }
  if (text.includes("downloadFile:fail")) {
    return "文件下载失败，请稍后重试"
  }
  if (text.includes("openDocument:fail")) {
    return "文件预览失败，请确认文件格式是否受支持"
  }
  return text
}

export function resolveFriendlyErrorFromPayload(payload, fallback = "操作失败", status) {
  if (!payload || typeof payload !== "object") {
    return STATUS_CODE_MESSAGES[status] || fallback
  }

  const codeMessage = getErrorMessageByCode(payload.code)
  if (codeMessage) {
    return codeMessage
  }
...
export function friendlyError(error, fallback = "操作失败") {
  ...
  if (error && typeof error === "object") {
    const resolved = resolveFriendlyErrorFromPayload(
      error,
      "",
      Number(error.status_code || error.statusCode || 0),
    )
    if (resolved) {
      return resolved
    }
  ...
  if (typeof error?.errMsg === "string" && error.errMsg.trim()) {
    return resolveNativeMiniProgramError(error.errMsg, fallback)
  }
